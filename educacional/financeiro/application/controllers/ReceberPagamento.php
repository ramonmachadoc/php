...
class ReceberPagamento extends CI_Controller {
    ...
    public function carregaPeriodoLetivo($param1 = '') {
        ...
        $ArrayPeridoLetivo = $this->financeiro_model->getPeriodoLetivo($param1);
    ...
    public function carregaTurma($param1 = '', $param2 = '', $param3 = '') {
        ...
        $DadosPeriodo = $this->financeiro_model->getTableRow('periodo_letivo', 'periodo_letivo', 'periodo_letivo_id', $param2);
        $periodo = $DadosPeriodo['periodo_letivo'];
        $ArrayTurma = $this->financeiro_model->getTurma($param1, $periodo = $periodo . "" . $param3);
    ...
    public function PesquisaAlunos($param1 = '', $param2 = '', $param3 = '', $param4 = '') {
        ...
        $arrayAlunos = $this->financeiro_model->GetPesquisaAlunos($param1, $param2, $nome, $param4);
        ...
        foreach ($arrayAlunos as $rowAlunos):
            ...
            //to-do provisorio solucao
            $status_turma = $this->financeiro_model->getPeriodoUltimoTurma($rowAlunos['matricula']);
            $situacao = $status_turma['situacao_aluno_turma'];
            ...
            echo '<td><a href="ReceberPagamento/pagamentosAlunos/' . $rowAlunos['matricula'] . '"><span class="label label-primary label-mini">Consultar Pagamentos</span></a></td>';
            echo '</tr>';
    ...
    public function carnePrint($param1 = '', $param2 = '') {
        ...
        $page_data['mensalidades'] = $this->financeiro_model->MensalidadesCarne($param2);
        $page_data['DadosAluno'] = $this->financeiro_model->DadosAlunoCarne($param1, $param2);
    ...
    public function reciboImpressao($param1 = '', $param2 = '', $param3 = '', $param4 = '') {
        ...
        $page_data['recibo'] = $this->financeiro_model->ReciboPagamento($param2, $param3, $param4);
        $page_data['DadosAluno'] = $this->financeiro_model->DadosAlunoCarne($param1, $param2);
    ...
    public function PagaMatricula($param1 = '', $param2 = '', $param3 = '') {
        ...
        if ($this->input->post()) {
            ...
            if ($mensalidade_id) {
                ...
                if ($this->input->post('forma_pagamento') == 2) {
                    $novaDatPag = date("Y-m-d", strtotime(date("Y-m-d", strtotime($data_pagamento)) . " +1 month"));
                } else {
                    $novaDatPag = $data_pagamento;
                }

                $dadosMovFin = array(
                    "mf_dt_pgto" => $novaDatPag,
                    "tipo" => 1,
        ...
        if ($this->input->post()) {
            ...
            for ($i = 1; $i <= $qtd_parcelas; $i++) {
                ...
                if ($this->input->post('forma_pagamento') <> 2) {
                    ...
                    $novaDatPag = date("Y-m-d", strtotime(date("Y-m-d", strtotime($dt_pgto)) . " +$i month"));
                }

                $dadosMensalidade = array(
                    "mf_dt_pgto" => $novaDatPag,
                    "mf_db_valor" => str_replace(',', '.', str_replace('.', '', $this->input->post('valor_pago')) / $qtd_parcelas),
                    ...
                    "mf_nb_forma_pagamento" => $this->input->post('forma_pagamento'),
                    "data_entrada" => $dt_pgto
                );

                $resultado = $this->financeiro_model->save('movimento_financeiro', $dadosMensalidade);
            }
            ...
            if ($resultado) {
                ...
                if ($this->financeiro_model->update($updateMensalide, 'mensalidade', $this->input->post('mensalidade_id'))) {
