...
$disable_element = $fullwidth = $parallax_bg = $boxed = $line = '';
$output = $after_output = '';
...
if ( ! empty( $img_halfrow ) ) {
	...
	$output .= '<div class="image-container ' . implode( ' ', $img_halfrow_attributes ) . '"><div class="background-image" style="background: url(' . esc_url($halfrow_image_src) . ') ' . esc_url($bgcolor) . ';"></div></div>';
}
if ($fullwidth == 'no') {
...
if($line == 'right'){
	$output .= '<div class="line-top-right w-680"></div>';
}
if($line == 'left'){
	$output .= '<div class="line-top-left w-240"></div>';
}
$output .= '</section>';
$output .= $after_output;
