...
( function($) {
    ...
    $.fn.wterm = function(options) {
        ...
        return this.each(function() {
            ...
            element.html('').append('<div id="container"></div>');
            var container = $('#container');

            container.html('').append('<div>' + settings.WELCOME_MESSAGE + '</div>');

            container.append('<div class="' + settings.CONTENT_CLASS + '"></div>');
            container.append('<div><span class="' + settings.PROMPT_CLASS + '">' + settings.PS1 + '&nbsp;</span>' +
                    '<form> <input type="text" ></form></div>');


            // Representing prompt, form, input and content section
            // in the terminal
            var _prompt = container.find('div:last span:last');
            var input_form = container.find('div:last form');
            var input = container.find('div:last form input');
            var content = container.find('.' + settings.CONTENT_CLASS);
            ...
            var update_content = function(p, cmd, data) {
                ...
                // scroll container down after rendering output
                var container = document.getElementById(settings.DEFAULT_THEME);
                // IE needs scrollTop reset
