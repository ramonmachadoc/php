...
function initQuickButton(element) {
    ...
    $jq(element + ' a.product-image').each(function(path) {
        ...
        if(rel == null) {
            ...
            paths[count] = subpath;
            $jq(this).closest("." + MA.QuickView.PARENT_ELEMENT).find("." + MA.QuickView.CHILDREN_ELEMENT).append('<li class="qv-button-container"><button type="button" ' +
                'class="qv-e-button btn-quickview-' + count + '">' +
                '<span><i class="fa fa-arrows"></i></span></button></li>');
