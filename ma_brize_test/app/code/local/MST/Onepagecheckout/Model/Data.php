<?php
class MST_OnepageCheckout_Model_Data
{
    ...
    //----------------------------------
    public function _checkChangedAddress($data, $addr_type = 'Billing', $addr_id = false, $check_city_street = false)
    {
    	$method	= "get{$addr_type}Address";
        $address = $this->getQuote()->{$method}();
        if(!$addr_id)
        {
    ...
    {
        // get data config default by kenvill
        $countryId = Mage::getStoreConfig('onepagecheckout/general/country');
        $countryId = $countryId?$countryId:Mage::getStoreConfig('general/country/default');            
        $default_city = Mage::getStoreConfig('onepagecheckout/general/city');
        $default_zip_postal = Mage::getStoreConfig('onepagecheckout/general/zip_postal');
        // end get data config
        $quote = $this->getQuote();
        // try to get shipping data
        $ship_data	= $quote->getShippingAddress()->getData();

        $init_ship_data	= array(
			'country_id'   => !empty($ship_data['country_id'])?$ship_data['country_id']:$countryId,
			'city'      => !empty($ship_data['city'])?$ship_data['city']:$default_city,
			'postcode'  => !empty($ship_data['postcode'])?$ship_data['postcode']:$default_zip_postal,
    ...
    public function saveOrder()
    {
        ...
        $this->getCheckout()->setProcessedOnepagecheckout('onepagecheckout');
        
        $service_quote = Mage::getModel('onepagecheckout/validate', $this->getQuote());
    ...
    public function validateAddress($address)
    {
        ...
        $address->implodeStreetAddress();
        $a_form = Mage::getStoreConfig('onepagecheckout/exclude');
        
        if (!Zend_Validate::is($address->getFirstname(), 'NotEmpty'))
            $errors[] = $helper->__('<br/>Please enter the first name.<br/>');

        if (!Zend_Validate::is($address->getLastname(), 'NotEmpty'))
            $errors[] = $helper->__('Please enter the last name.<br/>');
         
        $sessionCustomer = Mage::getSingleton("customer/session");    
        if (!Zend_Validate::is($address->getEmail(), 'NotEmpty') && !$sessionCustomer->isLoggedIn())
            $errors[] = $helper->__('Please enter the email.<br/>');   
            
        if (!$a_form['country'] && !Zend_Validate::is($address->getCountryId(), 'NotEmpty'))
            $errors[] = $helper->__('Please choose the country.<br/>');
        
        if (!$a_form['city'] && !Zend_Validate::is($address->getCity(), 'NotEmpty'))
            $errors[] = $helper->__('Please enter the city.<br/>');
        
        if (!$a_form['telephone'] && !Zend_Validate::is($address->getTelephone(), 'NotEmpty'))
            $errors[] = $helper->__('Please enter the phone number.<br/>');
                
        /*if (!$a_form['company'] && !Zend_Validate::is($address->getCompany(), 'NotEmpty'))
            $errors[] = $helper->__('Please enter the company.<br/>');
        
        if (!$a_form['fax'] && !Zend_Validate::is($address->getFax(), 'NotEmpty'))
            $errors[] = $helper->__('Please enter the fax.<br/>');*/
        
        if (!$a_form['region'] && $address->getCountryModel()->getRegionCollection()->getSize() && !Zend_Validate::is($address->getRegionId(), 'NotEmpty'))
            $errors[] = $helper->__('Please enter the state/province.<br/>');

        $_opt_zip = Mage::helper('directory')->getCountriesWithOptionalZip();
        if (!$a_form['zip_post'] && !in_array($address->getCountryId(), $_opt_zip) && !Zend_Validate::is($address->getPostcode(), 'NotEmpty'))
            $errors[] = $helper->__('Please enter the zip code.<br/>');
            
        if (!$a_form['address'] && !Zend_Validate::is($address->getStreet(1), 'NotEmpty'))
            $errors[] = $helper->__('Please enter the street.<br/>');
