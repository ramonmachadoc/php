<?php
class MST_Onepagecheckout_Block_Frontend_Checkout_Address extends Mage_Checkout_Block_Onepage_Billing
{
    ...
    public function buildCountriesSelectBox($addr_type)
    {
        ...
        $select = $this->getLayout()->createBlock('core/html_select')
        							->setId("{$addr_type}_country_id")->setName("{$addr_type}[country_id]")
									->setValue($countryId)->setOptions($this->getCountryOptions())
                                    ->setExtraParams('ajax="selectcountry"')
        ...
        if($required_country){$select->setClass('validate-select');}
